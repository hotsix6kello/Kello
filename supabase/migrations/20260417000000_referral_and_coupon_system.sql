-- ============================================================
-- Migration: Referral & Coupon System
-- ============================================================

-- 1. profiles 테이블에 referral_code 컬럼 추가
-- ============================================================
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS referral_code TEXT UNIQUE;

-- 랜덤 영숫자 6~8자리 코드 생성 함수
CREATE OR REPLACE FUNCTION public.generate_referral_code()
RETURNS TEXT
LANGUAGE plpgsql
AS $$
DECLARE
  chars  TEXT := 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; -- 혼동되는 문자(0,O,1,I) 제외
  code   TEXT := '';
  length INTEGER;
  i      INTEGER;
BEGIN
  -- 6~8자리 중 랜덤 길이 선택
  length := 6 + floor(random() * 3)::INTEGER;

  FOR i IN 1..length LOOP
    code := code || substr(chars, floor(random() * length(chars))::INTEGER + 1, 1);
  END LOOP;

  RETURN code;
END;
$$;

-- 신규 프로필 생성 시 referral_code 자동 부여 함수
CREATE OR REPLACE FUNCTION public.set_referral_code_on_profile_insert()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  new_code TEXT;
  attempts INTEGER := 0;
BEGIN
  -- UNIQUE 충돌에 대비해 최대 10회 재시도
  LOOP
    new_code := public.generate_referral_code();
    BEGIN
      NEW.referral_code := new_code;
      RETURN NEW;
    EXCEPTION WHEN unique_violation THEN
      attempts := attempts + 1;
      IF attempts >= 10 THEN
        RAISE EXCEPTION 'Failed to generate unique referral_code after 10 attempts';
      END IF;
    END;
  END LOOP;
END;
$$;

-- Trigger: profiles INSERT 시 자동 실행 (referral_code가 없을 때만)
DROP TRIGGER IF EXISTS trg_set_referral_code ON public.profiles;
CREATE TRIGGER trg_set_referral_code
  BEFORE INSERT ON public.profiles
  FOR EACH ROW
  WHEN (NEW.referral_code IS NULL)
  EXECUTE FUNCTION public.set_referral_code_on_profile_insert();

-- 기존 profiles 레코드 중 referral_code가 없는 행에 소급 적용
UPDATE public.profiles
SET    referral_code = public.generate_referral_code()
WHERE  referral_code IS NULL;


-- 2. coupons 테이블 신규 생성
-- ============================================================
CREATE TABLE IF NOT EXISTS public.coupons (
  id             UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id        UUID        NOT NULL
                             REFERENCES public.profiles(id) ON DELETE CASCADE,
  discount_type  VARCHAR(20) NOT NULL DEFAULT 'percent',
  discount_value INTEGER     NOT NULL DEFAULT 5,
  issue_reason   VARCHAR(50),          -- 'signup' | 'referred' | 'referrer'
  is_used        BOOLEAN     NOT NULL DEFAULT false,
  created_at     TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- coupons: Row Level Security
ALTER TABLE public.coupons ENABLE ROW LEVEL SECURITY;

-- 본인 쿠폰만 조회 가능
CREATE POLICY "coupons_select_own"
  ON public.coupons
  FOR SELECT
  USING (auth.uid() = user_id);

-- 서비스 역할(백엔드 / Edge Function)만 삽입 가능
CREATE POLICY "coupons_insert_service_role"
  ON public.coupons
  FOR INSERT
  WITH CHECK (auth.role() = 'service_role');


-- 3. referrals 테이블 신규 생성
-- ============================================================
CREATE TABLE IF NOT EXISTS public.referrals (
  id          UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  referrer_id UUID        NOT NULL
                          REFERENCES public.profiles(id) ON DELETE CASCADE,
  referred_id UUID        NOT NULL UNIQUE   -- 한 명은 단 한 번만 추천받을 수 있음
                          REFERENCES public.profiles(id) ON DELETE CASCADE,
  created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- referrals: Row Level Security
ALTER TABLE public.referrals ENABLE ROW LEVEL SECURITY;

-- 추천인 또는 피추천인 본인만 해당 레코드 조회 가능
CREATE POLICY "referrals_select_participants"
  ON public.referrals
  FOR SELECT
  USING (
    auth.uid() = referrer_id
    OR auth.uid() = referred_id
  );

-- 서비스 역할만 삽입 가능 (어뷰징 방지)
CREATE POLICY "referrals_insert_service_role"
  ON public.referrals
  FOR INSERT
  WITH CHECK (auth.role() = 'service_role');


-- 인덱스 (조회 성능)
CREATE INDEX IF NOT EXISTS idx_coupons_user_id       ON public.coupons(user_id);
CREATE INDEX IF NOT EXISTS idx_coupons_is_used        ON public.coupons(is_used);
CREATE INDEX IF NOT EXISTS idx_referrals_referrer_id  ON public.referrals(referrer_id);
CREATE INDEX IF NOT EXISTS idx_profiles_referral_code ON public.profiles(referral_code);
