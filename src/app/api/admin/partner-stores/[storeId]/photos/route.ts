...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ storeId: string }> }) {
  try {
    ...
    if (!photos || photos.length === 0 || !photos.every(isPartnerStoreBulkReviewPatchItem)) {
      return jsonFailure("photos must be a non-empty array of { id, review_status }", 400);
    }

    const client = getSupabaseServerClient();

    const results = await Promise.all(
      (photos as PartnerStoreBulkReviewPatchItem[]).map((photo) =>
        client
          .from("photos")
          .update({ review_status: photo.review_status })
          .eq("id", photo.id)
          .eq("store_id", storeId)
    ...
    if (failed?.error) {
      console.error("[admin-partner-store-photos] update_failed", failed.error.message);
      return jsonFailure("failed to update photos", 500);
    }
    ...
    return NextResponse.json({
      ok: true,
      photos: updated.map((row) => ({ id: row.id, reviewStatus: row.review_status })),
    });
  } catch (error) {
    const adminFailure = handleAdminRouteError(error, "admin-partner-store-photos");
    if (adminFailure) return adminFailure;
    console.error("[admin-partner-store-photos] unexpected_failure", error);
    return jsonFailure("failed to update photos", 500);
  }
}
